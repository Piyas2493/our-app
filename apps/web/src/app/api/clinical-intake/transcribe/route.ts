...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Speech generation is unavailable because GEMINI_API_KEY is not configured.",
        },
        { status: 503 },
      );
    }
    ...
    const prompt =
      ...
      `Do not translate it, summarize it, or change its meaning. ` +
      `Use natural ${languageName} pronunciation for India (${languageCode}) ` +
      `and a clear, calm clinical-education voice.\n\n` +
      text;
    ...
    if (!googleResponse.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: `Gemini speech generation failed (${googleResponse.status}).`,
    ...
    console.error("Clinical intake TTS route failed:", error);
